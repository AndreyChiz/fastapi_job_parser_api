...
schaduler_config = SchedulerConfig()
...
scheduler.add_job(update_data_job, 'interval', minutes=int(schaduler_config.UPDATE_FREQUENCY))
scheduler.start()
...
@app.get("/", description="Выводит список id и названий заказов.", response_model=List[OrderData])
async def print_orders(request: Request):
    data = await scrapper.get_data()
    if not data:
        raise HTTPException(status_code=404, detail="No orders found")
